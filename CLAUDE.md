...
2. **Run mechanical parser validation after every parser is generated.** After saving each parser in `tools/importer/parsers/`, execute `node tools/importer/validate-parser-content.js <parser-name>`. This script mechanically compares content elements (headings, paragraphs, links, images) in the source block element against what the parser's selectors actually cover. Do not proceed to the next parser or mark import infrastructure as complete until all discrepancies reported by the script are resolved. If the script reports uncovered elements, fix the parser selectors — never assume a content element is nested inside a container based on class naming alone (e.g., `.abde-home-hero-content` only contained the `<h1>`, not the sibling `<p>` in `.abde-pt-sm`). This exists because AI-based selector validation is unreliable and cannot replace mechanical verification.

3. **External images must be committed to GitHub for AEM resolution.** Images sourced from external domains (e.g., `alliancebernstein.com/content/dam/...`) cannot be resolved by the AEM backend when authored in DA — DA stores a local copy but AEM's publishing pipeline can't generate a media hash for inaccessible sources. This causes the entire table cell to be dropped (not just the image). Fix: download the image, place it in `/media/<pagename>/` with a `media_<hash>` filename (matching the existing pattern), commit and push to GitHub, then update the DA document to reference the repo-hosted path (e.g., `/media/home/media_<hash>.png`). The AEM CDN will then resolve it properly.

4. **Source site uses spacer utility classes for all vertical spacing.** The original AllianceBernstein site controls spacing between sections, blocks, and divider labels via `.abde-spacer` elements and utility margin/padding classes (`abde-mb-xl` = 160px, `abde-mb-lg` = 120px, `abde-mb-md` = 80px, `abde-mb-sm` = 40px, `abde-mb-xs` = 10px, `abde-mb-xxl` = 180px, `abde-mt-md` = 65px margin-top). When migrating spacing, always extract the computed values from these utility classes on the original — never guess spacing from screenshots alone. Watch out for CSS margin collapsing: adjacent sibling margins collapse to `max()`, not sum. Use `padding` instead of `margin` on one side when the full sum of two adjacent spacings is needed (e.g., 160px block margin-bottom + 65px divider padding-top = 225px visual gap without collapsing).
